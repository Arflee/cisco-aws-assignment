...
```bash
cisco-assignment$ sam build --use-container
```
...
```bash
cisco-assignment$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cisco-assignment$ sam local start-api
cisco-assignment$ curl http://localhost:3000/
```
...
```bash
cisco-assignment$ sam logs -n HelloWorldFunction --stack-name cisco-assignment --tail
```
...
```bash
sam delete --stack-name "cisco-assignment"
```
